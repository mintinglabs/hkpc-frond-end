...
export default function Home() {
  ...
  const handleMessage = (data: any) => {
    ...
    setMessages((prev) => [
      ...prev,
      `https://hkpc-app-service-169749647729.asia-east2.run.app${tData?.file?.filepath}`,
    ]);
  };
  ...
  useEffect(() => {
    const getSigList = async (type: 0 | 1, limit: number) => {
      ...
      const tData = res?.result?.map(
        (item: any) =>
          `https://hkpc-app-service-169749647729.asia-east2.run.app${item.file.filepath}`
      );
    ...
    <div className="text-white text-center text-[32px] font-[500]">
      ...
      <div className="flex flex-wrap gap-4">
        {messages.map((item, index) => (
          <div key={index}>
            <Image src={item} alt="sig" width={100} height={100} />
          </div>
        ))}
      </div>
    </div>
